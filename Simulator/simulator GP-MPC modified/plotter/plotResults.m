...
plot(X_sim(1,:)')
hold on
plot(max_t1_op*ones(N,1),'m--')
hold on
plot(min_t1_op*ones(N,1),'m--')
...
plot(X_sim(2,:)')
hold on
plot(max_t2_op*ones(N,1),'m--')
hold on
plot(min_t2_op*ones(N,1),'m--')
...
figure
ax(1) = subplot(4,1,1);
plot(KPI_sigma(startPlot:end))
title('Uncertainty KPI')

ax(2) = subplot(4,1,2);
plot(KPI_o(startPlot:end))
title('Overflow KPI')

ax(3) = subplot(4,1,3);
plot(KPI_s(startPlot:end))
title('Safety KPI')

ax(4) = subplot(4,1,4);
plot(KPI_u(startPlot:end))
title('Smoothening KPI')
linkaxes(ax, 'x')
